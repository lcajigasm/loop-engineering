---
description: Propose which pending goals can run concurrently; on Codex this emits a sequential fallback plan.
---
Run the loop-engineering `parallel` command.

Follow the canonical spec's `parallel` section exactly, including the Codex
